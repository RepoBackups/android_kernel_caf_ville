...
static void htc_headset_mgr_power_suspend(struct power_suspend *h)
{
	HS_DBG();
}

static void htc_headset_mgr_late_resume(struct power_suspend *h)
{
#ifdef HTC_HEADSET_CONFIG_QUICK_BOOT
	int state = 0;
	HS_DBG();
...
#ifdef CONFIG_POWERSUSPEND
	hi->power_suspend.suspend = htc_headset_mgr_power_suspend;
	hi->power_suspend.resume = htc_headset_mgr_late_resume;
	register_power_suspend(&hi->power_suspend);
#endif
