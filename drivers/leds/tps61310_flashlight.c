...
struct tps61310_data {
	...
	struct power_suspend		fl_power_suspend;
...
static void flashlight_late_resume(struct power_suspend *handler)
{
}
#endif
...
#ifdef CONFIG_POWERSUSPEND
	tps61310->fl_power_suspend.suspend = flashlight_power_suspend;
	tps61310->fl_power_suspend.resume  = flashlight_late_resume;
	register_power_suspend(&tps61310->fl_power_suspend);
#endif
...
static int tps61310_remove(struct i2c_client *client)
{
	...
	destroy_workqueue(tps61310_work_queue);
	mutex_destroy(&tps61310_mutex);
	unregister_power_suspend(&tps61310->fl_power_suspend);
	kfree(tps61310);
