...
#ifdef CONFIG_POWERSUSPEND
	dd->tdisc_power_suspend.level = EARLY_SUSPEND_LEVEL_BLANK_SCREEN +
						TDISC_SUSPEND_LEVEL;
	dd->tdisc_power_suspend.suspend = tdisc_power_suspend;
	dd->tdisc_power_suspend.resume = tdisc_late_resume;
	register_power_suspend(&dd->tdisc_power_suspend);
#endif
	return 0;
