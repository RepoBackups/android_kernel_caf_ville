...
static int is_power_suspended = false;
static int requested_in_power_suspend_lcd_level= 0;
#endif
...
void lm3533_backlight_on(int level)
{
	...
	if (is_power_suspended) {
		requested_in_power_suspend_lcd_level = level;
		return;
	}
#endif /* CONFIG_POWERSUSPEND */
...
void lm3533_power_suspend(struct power_suspend * h)
{
...
void lm3533_late_resume(struct power_suspend * h)
{
	...
	lm3533_lcd_backlight_set_level(requested_in_power_suspend_lcd_level);
	return;
}
#endif /* CONFIG_POWERSUSPEND */
...
#if defined(CONFIG_FB_MSM_MIPI_LGIT_CMD_WVGA_INVERSE_PT_PANEL) || \
	defined(CONFIG_FB_MSM_MIPI_LGIT_VIDEO_WVGA_INVERSE_PT_PANEL)
	power_suspend.suspend = lm3533_power_suspend;
	power_suspend.resume = lm3533_late_resume;
#else
	power_suspend.suspend = lm3533_backlight_off;
#endif
